	...
	while (!buildingsFile.eof())
	{
		...
		if (temporaryString == "BUILDING" && isAllocated == true)
		{
			...
			if (buildingID <= (m_numberOfBuilding - 1))
			{
				...
				if (temporaryString == "Stomping_Vats") m_buildings[buildingID].SetName(TransformStringToCenteredOne(temporaryString, 2));
				else m_buildings[buildingID].SetName(TransformStringToCenteredOne(temporaryString));
