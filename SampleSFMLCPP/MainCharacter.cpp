			...
			if (m_buildingTimer >= 2)
			{
					...
					&& _game->m_buildingsListPlanned->GetBuildingSize() != sf::Vector2i(-1, -1))
				{
					...
					case BUILDING_VINES:
						...
						storage = _game->m_builds.m_vines.GetStorage((sf::Vector2f)_game->m_buildingsListPlanned->GetBuildingPositionInMap());

						storage->AddNewResourceToStorage(Ressources::GetNameFromEnum(BUNCH_OF_GRAPE), ResourceData::RESOURCE_PRODUCED);
